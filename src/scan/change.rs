...
pub fn same(x: &Change, y: &Change) -> bool {
    same_with_file_contents(x, y, |d1, d2| match (d1.digest, d2.digest) {
        (Some(a), Some(b)) => a == b,
...
pub fn same_strong(x: &Change, y: &Change) -> bool {
    same_with_file_contents(
        x,
        y,
        |d1, d2| matches!((d1.digest, d2.digest), (Some(a), Some(b)) if a == b),
    )
}
